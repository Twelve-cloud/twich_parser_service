...
class GetTwichUserHandler(IQueryHandler[GetTwichUser, TwichUserDTO]):
    def __init__(
        self,
    ...
    async def handle(self, query: GetTwichUser) -> TwichUserDTO:
        user: TwichUser = await self.repository.get_by_id(query.id)
